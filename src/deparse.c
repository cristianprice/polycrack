...
bool is_returning, Bitmapset *attrs_used,
...
build_tlist_to_deparse(RelOptInfo *foreignrel) {
	 ...
	 */
	tlist = add_to_flat_tlist(tlist, pull_var_clause((Node *) foreignrel->reltarget->exprs,
...
static void deparseFromExpr(List *quals, deparse_expr_cxt *context) {
	...
	deparseFromExprForRel(buf, context->root, scanrel, (bms_num_members(scanrel->relids) > 1), (Index) 0, NULL, context->params_list);
...
bool use_alias, Index ignore_rel, List **ignore_conds, List **params_list) {
	PgFdwRelationInfo *fpinfo = (PgFdwRelationInfo *) foreignrel->fdw_private;
	...
	if (IS_JOIN_REL(foreignrel)) {
		...
		if (!outerrel_is_target) {
			...
			deparseRangeTblRef(&join_sql_o, root, outerrel, fpinfo->make_outerrel_subquery, ignore_rel, ignore_conds, params_list);
		...
		if (!innerrel_is_target) {
			...
			deparseRangeTblRef(&join_sql_i, root, innerrel, fpinfo->make_innerrel_subquery, ignore_rel, ignore_conds, params_list);
...
bool make_subquery, Index ignore_rel, List **ignore_conds, List **params_list) {
	PgFdwRelationInfo *fpinfo = (PgFdwRelationInfo *) foreignrel->fdw_private;
	...
	if (make_subquery) {
		...
		appendStringInfoChar(buf, '(');
		deparseSelectStmtForRel(buf, root, foreignrel, NIL, fpinfo->remote_conds, NIL, true, &retrieved_attrs, params_list);
		appendStringInfoChar(buf, ')');
...
bool doNothing, List *returningList, List **retrieved_attrs) {
	...
	deparseReturningList(buf, rte, rtindex, rel, rel->trigdesc && rel->trigdesc->trig_insert_after_row, returningList, retrieved_attrs);
}
...
void deparseUpdateSql(StringInfo buf, RangeTblEntry *rte, Index rtindex, Relation rel, List *targetAttrs,
	...
	deparseReturningList(buf, rte, rtindex, rel, rel->trigdesc && rel->trigdesc->trig_update_after_row, returningList, retrieved_attrs);
}
 ...
 *		by RETURNING (if any)
 */
void deparseDirectUpdateSql(StringInfo buf, PlannerInfo *root, Index rtindex, Relation rel, RelOptInfo *foreignrel,
		List *targetlist, List *targetAttrs, List *remote_conds, List **params_list, List *returningList,
		List **retrieved_attrs) {
	deparse_expr_cxt context;
	int nestlevel;
	bool first;
	...
	foreach(lc, targetAttrs)
	{
		...
		if (!tle)
			elog(ERROR, "attribute number %d not found in UPDATE targetlist", attnum);
	...
	if (foreignrel->reloptkind == RELOPT_JOINREL)
		...
		deparseReturningList(buf, rte, rtindex, rel, false, returningList, retrieved_attrs);
}
...
void deparseDeleteSql(StringInfo buf, RangeTblEntry *rte, Index rtindex, Relation rel, List *returningList,
	...
	deparseReturningList(buf, rte, rtindex, rel, rel->trigdesc && rel->trigdesc->trig_delete_after_row, returningList, retrieved_attrs);
}
 ...
 *		by RETURNING (if any)
 */
void deparseDirectDeleteSql(StringInfo buf, PlannerInfo *root, Index rtindex, Relation rel, RelOptInfo *foreignrel,
		List *remote_conds, List **params_list, List *returningList, List **retrieved_attrs) {
	deparse_expr_cxt context;
	...
	if (foreignrel->reloptkind == RELOPT_JOINREL)
		...
		deparseReturningList(buf, planner_rt_fetch(rtindex, root), rtindex, rel, false, returningList, retrieved_attrs);
}
...
static void deparseOpExpr(OpExpr *node, deparse_expr_cxt *context) {
	...
	/* Sanity check. */
	Assert((oprkind == 'r' && list_length(node->args) == 1) || (oprkind == 'l' && list_length(node->args) == 1) || (oprkind == 'b' && list_length(node->args) == 2));
